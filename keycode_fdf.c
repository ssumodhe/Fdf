...
/*   Updated: 2017/03/27 20:05:32 by ssumodhe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_key_init(t_data *data)
{
	...
	ft_putendl("Je rentre dans la fonction");
	gap = data->image->gap;

	ft_putendl("Je defini gap");
	data->image->img_h = (((data->map->height + 5) * 2) * gap * 0.3) + \
				((data->map->highest + fabs((double)data->map->lowest)) * (gap * 3 / 4));
	ft_putendl("Je defini img_h");
	if (data->map->width > data->map->height)
		data->image->img_w = (data->map->width + 1) * gap;
	else if (data->map->width <= data->map->height)
		data->image->img_w = (data->map->height + 1) * gap;
	ft_putendl("Je defini img_w");
	data->image->x_orig = gap;
	ft_putendl("Je defini x_orig");
	data->image->y_orig = (data->image->img_h - ((data->map->height + 5) * gap * 0.3)) - \
