...
/*   Updated: 2017/04/01 18:09:11 by ssumodhe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_createwindow(t_data *data, t_image *image)
{
	...
	if (image->img_h >= (GAP + 345))
		image->fdf.win = mlx_new_window(image->fdf.mlx, image->img_w + \
				(GAP * 2) + 250, image->img_h + (GAP * 2), WIN_NAME);
	else
		image->fdf.win = mlx_new_window(image->fdf.mlx, image->img_w + \
				(GAP * 2) + 250, ((GAP * 2) + 345) + (GAP * 2), WIN_NAME);
